...
set -x
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS -c '{"function":"AddUser","Args":["1023","sienna","register"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt
sleep 3
...
infoln "TEST1 : Invoking the chaincode"
set -x
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS -c '{"function":"AddContract","Args":["1023","complete","1","testdoc","101011"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt
sleep 3
...
infoln "TEST1 : Invoking the chaincode"
set -x
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS -c '{"function":"UpdateState","Args":["1023","reject"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt
sleep 3
...
set -x
peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"function":"ReadContract","Args":["1023"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt